...
public class MlPredictionService : IMlPredictionService
{
    ...
    public MlPredictionService()
    {
        ...
        var pipeline = ml.Transforms.Concatenate("Features", nameof(ModelInput.MovementsCount), nameof(ModelInput.SectorChanges7d), nameof(ModelInput.HoursSinceLast))
    ...
    {
        var pred = _engine.Predict(new ModelInput(input.MovementsCount, input.SectorChangesLast7Days, input.HoursSinceLastMove, false));
        return new MovementPredictionDto(pred.Score > 0, pred.Score, pred.Probability);
    }

    private sealed record ModelInput(float MovementsCount, float SectorChanges7d, float HoursSinceLast, bool Label);
    private sealed class ModelOutput { public bool PredictedLabel { get; set; } public float Score { get; set; } public float Probability { get; set; } }
}